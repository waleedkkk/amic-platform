...
function AlertCenterView({ demoMode = false }: { demoMode?: boolean }) {
  ...
  return <><PageHeading eyebrow={demoMode ? "LOCAL DEMO — ALERT REVIEW CENTER" : "ALERT REVIEW CENTER"} title="مركز متابعة التنبيهات" description={demoMode ? "هذه معاينة محلية ببيانات افتراضية داخل المتصفح؛ لا تتصل بالسوق أو الخادم ولا تغير بيانات أي حساب." : "رشّح الأحداث، راجع التنبيه المرتبط بالأصل عند الحاجة، ثم علّمه مقروءًا. السجل يوثق ما تحقق من شروطك ولا يقدّم توصية."} />
    ...
    <Panel><div className="flex flex-wrap items-end gap-3"><div className="min-w-44"><div className="mb-2 flex items-center gap-1"><p className="text-xs text-muted-foreground">نوع التنبيه</p><ContextHelp term="أنواع التنبيه"><p>المعادن ترصد شروط أسعار المعادن، والبنية ترصد أحداث بنية السعر، والسياق يربط الحدث بالمستويات أو المناطق ذات الصلة.</p></ContextHelp></div><Select value={category} onValueChange={value => setCategory(value as Category)}><SelectTrigger className="bg-white/[0.025]"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(labels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><div className="min-w-48 flex-1"><p className="mb-2 text-xs text-muted-foreground">الرمز</p><Input value={symbol} onChange={event => setSymbol(event.target.value.toUpperCase())} placeholder="مثال: XAUUSD" className="bg-white/[0.025] font-mono" /></div><div className="flex items-center gap-1"><Button type="button" variant={unreadOnly ? "default" : "outline"} className="bg-white/[0.03]" onClick={() => setUnreadOnly(value => !value)}><Filter className="ml-1.5 size-4" />غير المقروءة فقط</Button><ContextHelp term="غير المقروءة"><p>تبقي هذه الفلترة التنبيهات التي لم تُعلّم مقروءة فقط. لا تعني أن التنبيه أحدث من غيره أو أهم منه بالضرورة.</p></ContextHelp></div></div>
      ...
      <div className="mt-5 flex gap-2 rounded-xl border border-amber-400/15 bg-amber-400/[0.04] p-3 text-xs leading-5 text-muted-foreground"><ShieldAlert className="mt-0.5 size-4 shrink-0 text-amber-300" /><p>التنبيه يوضح تحقق شرطك فقط؛ راجع السياق قبل اتخاذ أي قرار تدريبي.</p></div>
    </Panel>
  </>;
}
