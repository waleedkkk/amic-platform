...
function LayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  return <>
    <div className="relative" ref={sidebarRef}>
      <Sidebar collapsible="icon" className="border-l border-white/[0.07] bg-[#0c141e]" disableTransition={isResizing}>
        <SidebarHeader className="h-[76px] justify-center px-3"><div className="flex w-full items-center gap-3"><button onClick={toggleSidebar} className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.025] text-muted-foreground transition-colors hover:bg-white/[0.07]" aria-label="طي القائمة"><PanelLeft className="size-4" /></button>{!collapsed && <div className="min-w-0"><p className="text-base font-semibold tracking-tight">AMIC</p><p className="text-[10px] tracking-[0.15em] text-primary">MARKET INTELLIGENCE</p></div>}</div></SidebarHeader>
        <SidebarContent className="px-2 pt-2"><p className="mb-2 px-3 text-[10px] font-semibold tracking-[0.14em] text-muted-foreground group-data-[collapsible=icon]:hidden">مساحة العمل</p><SidebarMenu>{menuItems.map(item => <SidebarMenuItem key={item.path}><SidebarMenuButton isActive={location === item.path} onClick={() => setLocation(item.path)} tooltip={item.label} className="h-11 rounded-xl text-[13px] font-medium data-[active=true]:bg-primary/12 data-[active=true]:text-primary"><item.icon className="size-[18px]" /><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarContent>
        <SidebarFooter className="p-3"><div className="mb-3 rounded-xl border border-primary/15 bg-primary/[0.045] p-3 group-data-[collapsible=icon]:hidden"><p className="text-xs font-medium text-primary">قراءة مسؤولة</p><p className="mt-1 text-[11px] leading-5 text-muted-foreground">المعلومات تحليلية وتعليمية، وليست توصية استثمارية.</p></div><DropdownMenu><DropdownMenuTrigger asChild><button className="flex w-full items-center gap-3 rounded-xl p-2 text-right transition-colors hover:bg-white/[0.05] group-data-[collapsible=icon]:justify-center"><Avatar className="size-8 border border-white/10"><AvatarFallback className="bg-secondary text-xs">{user?.name?.charAt(0).toUpperCase() ?? "U"}</AvatarFallback></Avatar><div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden"><p className="truncate text-xs font-medium">{user?.name || "مستخدم AMIC"}</p><p className="mt-0.5 truncate text-[10px] text-muted-foreground">مساحة تحليل خاصة</p></div></button></DropdownMenuTrigger><DropdownMenuContent align="start" className="w-48"><DropdownMenuItem onClick={logout} className="cursor-pointer text-destructive focus:text-destructive"><LogOut className="ml-2 size-4" />تسجيل الخروج</DropdownMenuItem></DropdownMenuContent></DropdownMenu></SidebarFooter>
      </Sidebar>
      <div onMouseDown={() => !collapsed && setIsResizing(true)} className={cnResize(collapsed)} />
    </div>
    <SidebarInset className="surface-grid min-w-0 overflow-x-hidden bg-background"><div className="min-h-screen min-w-0 bg-[radial-gradient(circle_at_70%_0%,rgba(48,121,145,0.12),transparent_32%)]">{isMobile && <div className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-white/[0.07] bg-background/90 px-3 backdrop-blur"><SidebarTrigger className="rounded-lg bg-white/[0.05]" /><span className="text-sm font-medium">{active?.label ?? "AMIC"}</span></div>}<main className="mx-auto w-full min-w-0 max-w-[1600px] overflow-x-hidden p-4 sm:p-6 lg:p-8">{children}</main></div></SidebarInset>
  </>;
}
