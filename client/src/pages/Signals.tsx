...
import { ArrowDownRight, ArrowUpRight, BookmarkCheck, Copy, Share2, Trash2 } from "lucide-react";
...
export default function Signals() {
  ...
  const share = trpc.signals.share.useMutation({ onSuccess: result => { if (!result?.publicShareId) return toast.error("تعذر إنشاء رابط المشاركة لهذه الإشارة."); const url = `${window.location.origin}/signal/${result.publicShareId}`; void navigator.clipboard?.writeText(url); toast.success("أُنشئ رابط عام ونسخ إلى الحافظة."); utils.signals.list.invalidate(); }, onError: error => toast.error(error.message) });
