...
export default function PaperTrading() {
  ...
  return <>
    <PageHeading eyebrow="PAPER PORTFOLIO" title="التداول الورقي" description="محاكاة تعليمية فقط: اختر الأصل، حدّد خطة المخاطرة، راجع التحذيرات، ثم افتح أو أغلق الصفقة داخل حسابك. لا تُرسل أوامر حقيقية." action={<PaperTradeAlertCenter alerts={alerts} status={paperTradeSocketStatus} onDismiss={dismissAlert} onClear={clearAlerts} onSelect={handleAlertSelect} />} />
    ...
    <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      <Panel><div className="flex items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary/12 text-primary"><WalletCards className="size-5" /></div><div><h2 className="font-semibold">فتح صفقة محاكاة</h2><p className="text-xs text-muted-foreground">راجع كل القيم قبل الفتح؛ هي للتدريب والتوثيق فقط.</p></div></div>
        <form className="mt-6 grid gap-4 sm:grid-cols-2" onSubmit={submit}>
          ...
          <div><div className="flex items-center gap-1"><Label>الاتجاه</Label><ContextHelp term="اتجاه الصفقة"><p>شراء/Long يعني أن نتيجة المحاكاة تتحسن إذا ارتفع السعر، وبيع/Short يعني أنها تتحسن إذا انخفض السعر. لا يغيّر هذا أي رصيد حقيقي.</p></ContextHelp></div><Select value={form.side} onValueChange={value => updateForm({ side: value as TradeForm["side"] })}><SelectTrigger className="mt-2 bg-white/[0.025]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="long">شراء / Long</SelectItem><SelectItem value="short">بيع / Short</SelectItem></SelectContent></Select></div>
          ...
          <div><div className="flex items-center gap-1"><Label>وقف الخسارة (اختياري)</Label><ContextHelp term="وقف الخسارة"><p>سعر مرجعي في خطة المحاكاة يوضح الحد الذي تعتبر عنده الفكرة غير صالحة. يتحقق النموذج من منطقيته وفق اتجاه الصفقة.</p></ContextHelp></div><Input inputMode="decimal" className="mt-2 bg-white/[0.025] font-mono" value={form.stopLoss} onChange={event => updateForm({ stopLoss: event.target.value })} /></div>
          <div><div className="flex items-center gap-1"><Label>جني الربح (اختياري)</Label><ContextHelp term="جني الربح"><p>سعر مستهدف في خطة المحاكاة. عند إدخاله مع سعر الدخول ووقف الخسارة، تظهر نسبة العائد إلى المخاطرة أدناه.</p></ContextHelp></div><Input inputMode="decimal" className="mt-2 bg-white/[0.025] font-mono" value={form.takeProfit} onChange={event => updateForm({ takeProfit: event.target.value })} /></div>
          <div className="sm:col-span-2 rounded-xl border border-sky-400/20 bg-sky-400/[0.06] p-3" aria-live="polite"><div className="flex items-start gap-2"><Target className="mt-0.5 size-4 shrink-0 text-sky-300" /><div><div className="flex items-center gap-1"><p className="text-sm font-semibold text-sky-100">نسبة العائد إلى المخاطرة</p><ContextHelp term="نسبة العائد إلى المخاطرة"><p>تقارن العائد المحتمل بالمخاطرة المحتملة وفق أسعار خطتك فقط؛ لا تتنبأ بنتيجة الصفقة ولا تضمنها.</p></ContextHelp></div><p className="mt-1 font-mono text-lg text-sky-200">{riskAssessment.riskRewardRatio ? `${riskAssessment.riskRewardRatio.toFixed(2)} : 1` : "—"}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{riskAssessment.hasCompletePlan ? "العائد المحتمل لكل وحدة مخاطرة، بناءً على القيم التي أدخلتها." : "أدخل سعر الدخول ووقف الخسارة وجني الربح بقيم منطقية لعرض النسبة."}</p></div></div></div>
