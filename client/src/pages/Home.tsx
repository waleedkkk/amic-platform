...
import { DataTable, EmptyAction, formatValue, PageHeading, Panel, SignalBadge } from "@/components/market-ui";
...
const PULSE_WIDGETS: Array<{ key: MarketPulseWidgetKey; title: string; description: string }> = [
  { key: "summary", title: "ملخص الأقسام المختارة", description: "مؤشر مختصر للأقسام قبل القوائم التفصيلية" },
...
export default function Home() {
  ...
  return <>
    ...
    {activeWidgets.includes("summary") ? <Panel className="mb-6 p-3.5"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">ملخص سريع للأقسام:</span>{visibleSectionQueries.map(({ definition, query }) => { const response = isOverviewSlice(query.data) ? query.data : undefined; const positive = definition.direction === "gainers"; return <span key={definition.key} className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs ${positive ? "border-emerald-400/20 bg-emerald-400/[0.06] text-emerald-100" : "border-rose-400/20 bg-rose-400/[0.06] text-rose-100"}`}>{positive ? <TrendingUp className="size-3" /> : <TrendingDown className="size-3" />}{definition.title}: {query.isLoading ? "جارٍ الجلب" : query.isError ? "غير متاح" : `${formatValue(toMarketRows(response?.items).length, 0)} نتيجة`}</span>; })}</div><p className="mt-2 text-xs text-muted-foreground">التفاصيل والأسعار تظهر في القوائم أدناه؛ هذا السطر يوضح فقط ما المتاح الآن.</p></Panel> : null}
